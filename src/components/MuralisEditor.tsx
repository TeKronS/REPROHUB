...
export default function MuralisEditor() {
  ...
  return (
    <div className="flex flex-col h-screen w-full font-body bg-background">
      {/* Top Navbar */}
      <header className="h-14 border-b border-border bg-card/50 backdrop-blur-xl flex items-center justify-between px-6 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="default" className="bg-primary hover:bg-primary/90 text-white font-bold gap-2 px-6 shadow-lg shadow-primary/20" onClick={handleExport}>
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Central Canvas Area */}
        <section className="flex-1 relative bg-[#050a06] overflow-hidden flex items-center justify-center">
          {!image ? (
        ...
        <aside className="w-80 border-l border-border bg-card/50 backdrop-blur-md overflow-y-auto custom-scrollbar">
          <div className="p-6 space-y-8">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-headline font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                <Settings2 className="h-4 w-4" /> {t.gridSettings}
              </h2>
              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => setImage(null)}>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 space-y-4">
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-medium cursor-pointer" htmlFor="guides-switch">
                    {t.guides}
                  </Label>
                  <Switch 
                    id="guides-switch" 
            ...
            <div className="p-4 bg-primary/10 border border-primary/20 rounded-xl space-y-3">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-headline font-black text-white">{rows * cols}</span>
          ...
          <div className="p-6 mt-auto">
             <Button variant="outline" className="w-full border-primary/50 text-primary hover:bg-primary/5 font-bold uppercase text-xs tracking-widest py-6" onClick={() => setImage(null)}>
              {t.reset}
            </Button>
          </div>
        </aside>
      ...
      {/* Bottom Status Bar */}
      <footer className="h-8 border-t border-border bg-card/80 px-4 flex items-center justify-between text-[10px] text-muted-foreground font-mono uppercase tracking-widest z-50">
        <div className="flex gap-4">
          <span className="flex items-center gap-1"><Settings className="h-3 w-3" /> System: Stable</span>
          <span className="flex items-center gap-1"><Grid3X3 className="h-3 w-3" /> Engine: V2.5</span>
        </div>
        <div className="flex gap-4">
          <span>Resolution: 300 DPI</span>
          <span className="text-primary font-bold">Draft Mode: Off</span>
        </div>
      </footer>
    </div>
  );
